...
public class GenLayerShoreRTG extends GenLayer
{
    public GenLayerShoreRTG(long seed, GenLayer parent)
    {
        super(seed);
        ...
        for (int i1 = 0; i1 < length; ++i1)
        {
            for (int j1 = 0; j1 < width; ++j1)
            {
                ...
                else if (landBiomeId != BiomeGenBase.extremeHills.biomeID && landBiomeId != BiomeGenBase.extremeHillsPlus.biomeID && landBiomeId != BiomeGenBase.extremeHillsEdge.biomeID)
                {
                    ...
                    {
                        if (!RTGBiomeManager.isBiomeOceanic(landBiomeId) && landBiomeId != BiomeGenBase.ocean.biomeID && landBiomeId != BiomeGenBase.deepOcean.biomeID && landBiomeId != BiomeGenBase.river.biomeID && landBiomeId != BiomeGenBase.swampland.biomeID)
                        {
                            l1 = inputBiomeIds[j1 + 1 + (i1 + 1 - 1) * (width + 2)];
                            i2 = inputBiomeIds[j1 + 1 + 1 + (i1 + 1) * (width + 2)];
                            j2 = inputBiomeIds[j1 + 1 - 1 + (i1 + 1) * (width + 2)];
    ...
    {
    	return RTGBiomeManager.isBiomeOceanic(biomeId) || GenLayer.isBiomeOceanic(biomeId);
    }
