...
public class RealisticBiomeAddonDarkRedwoodPlains extends RealisticBiomeBase
{
	private SurfaceBase surface;

	public RealisticBiomeAddonDarkRedwoodPlains() 
	{
		super(
				RealisticBiomeAddonBase.darkRedwoodPlains,
				BiomeBase.climatizedBiome(BiomeGenBase.river, BiomeBase.Climate.COLD),
				new TerrainGrasslandFlats(),
				new SurfaceMountainStoneMix1(Blocks.grass, Blocks.dirt, false, null, 0f, 1.5f, 60f, 65f, 1.5f, Blocks.stone, 0.15f)
				);
		
		this.setRealisticBiomeName("Dark Redwood Plains");
		this.biomeSize = BiomeSize.NORMAL;
		this.biomeWeight = ConfigAddon.weightDarkRedwoodPlains;
		this.generateVillages = ConfigAddon.villageDarkRedwoodPlains;	
	}
    ...
    public void rDecorate(World world, Random rand, int chunkX, int chunkY, OpenSimplexNoise perlin, CellNoise cell, float strength, float river)
    {
		...
		if(l > 5f)
		{
			for (int b2 = 0; b2 < 3f * strength; b2++)
			{
				...
				if(z52 < 120)
				{
					WorldGenerator worldgenerator = rand.nextInt(4) == 0 ? new WorldGenTreePineSmall(4 + rand.nextInt(7), 6 + rand.nextInt(9), 0) /*: rand.nextInt(6) != 0 ? new WorldGenTreePineSmall(false)*/ : new WorldGenForest(false, false);
					worldgenerator.setScale(1.0D, 1.0D, 1.0D);
					worldgenerator.generate(world, rand, j6, z52, k10);
				}
			}
