package rtg.world.biome.realistic.environs;

import net.minecraft.world.biome.Biome;
import rtg.api.world.RTGWorld;
import rtg.api.world.terrain.TerrainBase;
import rtg.world.biome.realistic.vanilla.RealisticBiomeVanillaForestHills;


public class RealisticBiomeENVPineTaiga extends RealisticBiomeENVBase {

    public RealisticBiomeENVPineTaiga(Biome biome) {
        super(biome, BeachType.STONE);
    }

    @Override
    public void initConfig() {

    }

    @Override
    public TerrainBase initTerrain() {

        return new RealisticBiomeVanillaForestHills.TerrainVanillaForestHills();
    }

    public static class TerrainVanillaForest extends TerrainBase {

        private float hillStrength = 10f;

        public TerrainVanillaForest() {

        }

        @Override
        public float generateNoise(RTGWorld rtgWorld, int x, int y, float border, float river) {
            groundNoise = groundNoise(x, y, groundVariation, rtgWorld);
            float m = hills(x, y, hillStrength, rtgWorld);
            float floNoise = 65f + groundNoise + m;
            return riverized(floNoise, river);
        }
    }
}
