package teamrtg.rtg.world.gen.feature.tree.abyssalcraft;
...
public class WorldGenTreeACDarkwood extends WorldGenerator {
    ...
    public boolean generate(World world, Random rand, BlockPos pos) {
        ...
        Block g = world.getBlockState(pos).getBlock();
        if (g != ACBlocks.darklands_grass) {
            return false;
        }

// TODO: straighten this out later -srs_bsns
        int x = pos.getX();
        int y = pos.getY();
        int z = pos.getZ();
        ...
        for (i = 0; i < startHeight; i++) {
            world.setBlockState(new BlockPos(x, y, z), ACBlocks.darklands_oak_wood.getDefaultState(), metadataLog);
    ...
    public void buildTrunk(World world, Random rand, int x, int y, int z) {
        ...
        for (int i = -1; i < h; i++) {
            world.setBlockState(new BlockPos(x, y + i, z), ACBlocks.darklands_oak_wood.getDefaultState(), metadataLog);
        }
    }
    ...
    public void buildBranch(World world, Random rand, int x, int y, int z, int dX, int dZ, int logLength, int leaveSize) {
        ...
        for (int m = 1; m <= logLength; m++) {
            world.setBlockState(new BlockPos(x + (dX * m), y, z + (dZ * m)), ACBlocks.darklands_oak_wood.getDefaultState(), metadataLog);
        }
    }
