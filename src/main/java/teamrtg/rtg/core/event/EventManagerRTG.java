...
public class EventManagerRTG {
    ...
    private final IEventHandlerRTG GENERATE_MINEABLE_RTG = new IEventHandlerRTG() {
        {
        ...
        public void generateMinableRTG(OreGenEvent.GenerateMinable event) {
            ...
            if (Mods.RTG.config.GENERATE_ORES.get()) {
                ...
                switch (event.getType()) {
                    ...
                    case COAL:
                        if (!Mods.RTG.config.GENERATE_ORES_COAL.get()) {
                            event.setResult(Result.DENY);
                        }
                        return;

                    case DIAMOND:
                        if (!Mods.RTG.config.GENERATE_ORES_DIAMOND.get()) {
                            event.setResult(Result.DENY);
                        }
                        return;

                    case DIRT:
                        if (!Mods.RTG.config.GENERATE_ORES_DIRT.get()) {
                            event.setResult(Result.DENY);
                        }
                        return;

                    case GOLD:
                        if (!Mods.RTG.config.GENERATE_ORES_GOLD.get()) {
                            event.setResult(Result.DENY);
                        }
                        return;

                    case GRAVEL:
                        if (!Mods.RTG.config.GENERATE_ORES_GRAVEL.get()) {
                            event.setResult(Result.DENY);
                        }
                        return;

                    case IRON:
                        if (!Mods.RTG.config.GENERATE_ORES_IRON.get()) {
                            event.setResult(Result.DENY);
                        }
                        return;

                    case LAPIS:
                        if (!Mods.RTG.config.GENERATE_ORES_LAPIS.get()) {
                            event.setResult(Result.DENY);
                        }
                        return;

                    case REDSTONE:
                        if (!Mods.RTG.config.GENERATE_ORES_REDSTONE.get()) {
                            event.setResult(Result.DENY);
                        }
                        return;

                    case DIORITE:
                        if (!Mods.RTG.config.GENERATE_ORES_DIORITE.get()) {
                            event.setResult(Result.DENY);
                        }
                        return;

                    case GRANITE:
                        if (!Mods.RTG.config.GENERATE_ORES_GRANITE.get()) {
                            event.setResult(Result.DENY);
                        }
                        return;

                    case ANDESITE:
                        if (!Mods.RTG.config.GENERATE_ORES_ANDESITE.get()) {
                            event.setResult(Result.DENY);
                        }
                        return;

                    case EMERALD:
                        if (!Mods.RTG.config.GENERATE_ORES_EMERALD.get()) {
                            event.setResult(Result.DENY);
                        }
                        return;
    ...
    private final IEventHandlerRTG INIT_BIOME_GENS_RTG = new IEventHandlerRTG() {
        {
    ...
    private final IEventHandlerRTG INIT_MAP_GEN_RTG = new IEventHandlerRTG() {
        {
    ...
    private final IEventHandlerRTG SAPLING_GROW_TREE_RTG = new IEventHandlerRTG() {
        {
        ...
        public void saplingGrowTreeRTG(SaplingGrowTreeEvent event) {
            ...
            RTGBiome rb = RTGBiome.forBiome(BiomeUtils.getId(vb));
    ...
    private final IEventHandlerRTG LOAD_CHUNK_RTG = new IEventHandlerRTG() {
        {
    ...
    private final IEventHandlerRTG RTG_EVENT_REGISTER = new IEventHandlerRTG() {
        {
        ...
        public void registerRTGEventHandlers(WorldTypeEvent.InitBiomeGens event) {
            ...
            if (event.getWorldType() instanceof WorldTypeRTG) {
                if (!registered) {
                    ...
                    if (registered)
                        Logger.info(EVENT_SYSTEM + "RTG's Terrain Event Handlers have been registered successfully.");
                }
            } else {
    ...
    @SuppressWarnings("FieldCanBeLocal")
    private final IEventHandlerRTG WORLD_EVENT_RTG = new IEventHandlerRTG() {
        {
    ...
    public void registerEventHandlers() {
        ...
        MinecraftForge.ORE_GEN_BUS.register(GENERATE_MINEABLE_RTG);
    ...
    public void unRegisterEventHandlers() {
        ...
        MinecraftForge.ORE_GEN_BUS.unregister(GENERATE_MINEABLE_RTG);
    ...
    private interface IEventHandlerRTG {
    }
}