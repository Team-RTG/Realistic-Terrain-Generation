...
public class TerrainDunes extends TerrainBase {
    ...
    public float generateNoise(ChunkProviderRTG provider, int x, int y, float border, float river) {
        float st = (provider.simplex.noise2(x / 160f, y / 160f) + 0.38f) * (Mods.RTG.config.DUNE_HEIGHT.get() + 23f);
        st = st < 0.2f ? 0.2f : st;
        ...
        if (h < 10f) {
            float d = (h - 10f) / 2f;
            d = d > 4f ? 4f : d;
            h += provider.cell.noise(x / 25D, y / 25D, 1D) * d;
            h += provider.simplex.noise2(x / 30f, y / 30f) * d;
            h += provider.simplex.noise2(x / 14f, y / 14f) * d * 0.5f;
        }
