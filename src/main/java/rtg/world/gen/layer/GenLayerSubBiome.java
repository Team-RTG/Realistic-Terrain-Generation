	...
	public int[] getInts(int x, int z, int width, int length)
    {
        ...
        for (int xi = 0; xi < width; ++xi) 
        {
        	for (int zi = 0; zi < length; ++zi) 
        	{
        		...
        		//List<BiomeEntry> currentSubBiomes = RTGBiomeManager.overworldSubBiomes[currentBiomeId];
        		//RTGSubBiome selectedSubBiome = currentSubBiomes != null ? (RTGSubBiome)currentSubBiomes.get(this.nextInt(currentSubBiomes.size())).biome : null;
        		
        		//if (selectedSubBiome != null)
        		//{
        		//	Pair<Integer, Integer> offset = getOffset(selectedSubBiome);
        		//	
        		//	if (PerlinNoise.noise((xi + x + offset.getLeft()) * selectedSubBiome.zoom, (zi + z + offset.getRight()) * selectedSubBiome.zoom) > selectedSubBiome.threshold)  
        		//	{
