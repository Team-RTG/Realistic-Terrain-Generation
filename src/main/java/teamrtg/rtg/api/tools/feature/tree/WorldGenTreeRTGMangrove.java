...
public class WorldGenTreeRTGMangrove extends WorldGenerator {
    ...
    public boolean generate(World world, Random rand, int x, int y, int z) {
        ...
        if (b != GRASS && b != DIRT && b != SAND) {
            if (!(b == WATER && world.getBlockState(new BlockPos(x, y - 2, z)).getBlock() == SAND && world.getBlockState(new BlockPos(x, y, z)).getBlock() == AIR)) {
                return false;
            }
        }
    ...
    public void generateLeaves(World world, Random rand, int x, int y, int z, float size, float width) {
        float dist;
        int i, j, k, s = (int) (4f - 1f), w = (int) ((4f - 1f) * 1.2f);
        for (i = -w; i <= w; i++) {
            for (j = -s; j <= s; j++) {
                for (k = -w; k <= w; k++) {
                    dist = abs((float) i / 1.2f) + (float) abs(j) + abs((float) k / 1.2f);
