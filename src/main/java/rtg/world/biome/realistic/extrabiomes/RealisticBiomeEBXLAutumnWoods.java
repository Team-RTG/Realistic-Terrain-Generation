...
{	
	...
	public static Block logBlock = Block.getBlockFromName("ExtrabiomesXL:log2");
    ...
    public void rDecorate(World world, Random rand, int chunkX, int chunkY, OpenSimplexNoise simplex, CellNoise cell, float strength, float river)
    {
        ...
        if (this.config.getPropertyById(BiomeConfigEBXLAutumnWoods.decorationLogsId).valueBoolean) {
            ...
            if (rand.nextInt(24) == 0)
            {
                ...
                {
                    if (rand.nextBoolean()) {
                        (new WorldGenLog(logBlock, 1, Blocks.leaves, -1, 2 + rand.nextInt(2))).generate(world, rand, new BlockPos(x22, y22, z22));
                    }
                    else {
                        (new WorldGenLog(Blocks.log, 0, Blocks.leaves, -1, 2 + rand.nextInt(2))).generate(world, rand, new BlockPos(x22, y22, z22));
                    }
                }
            }
        }
