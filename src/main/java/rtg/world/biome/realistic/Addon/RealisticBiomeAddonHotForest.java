...
public class RealisticBiomeAddonHotForest extends RealisticBiomeBase
{
	private TerrainBase terrain;
	private SurfaceBase surface;
	
	public RealisticBiomeAddonHotForest() 
	{
		super(
		...
		this.setRealisticBiomeName("Hot Forest");
		this.biomeSize = BiomeSize.NORMAL;
		this.biomeWeight = ConfigAddon.weightHotForest;
		this.generateVillages = ConfigAddon.villageHotForest;	
	}
    ...
    public void rDecorate(World world, Random rand, int chunkX, int chunkY, OpenSimplexNoise perlin, CellNoise cell, float strength, float river)
    {
    	...
    	for(int b = 0; b < 2f * strength; b++)
    	{
		    ...
		    {
    		   (new WorldGenTreeShrub(rand.nextInt(5) + 4, rand.nextInt(2), 0)).generate(world, rand, i1, k1, j1);
		    }
		    else
		    {
		    	(new WorldGenTreeShrub(rand.nextInt(4) + 1, rand.nextInt(2), 0)).generate(world, rand, i1, k1, j1);
		    }
    	}
