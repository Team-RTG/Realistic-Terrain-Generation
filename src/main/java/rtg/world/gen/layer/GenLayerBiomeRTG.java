...
public class GenLayerBiomeRTG extends GenLayerBiome
{
	...
	public GenLayerBiomeRTG(long seed, GenLayer parentLayer, WorldType worldType) 
	{
		...
		this.biomeLists[0].addAll(RTGBiomeManager.overworldBiomes[0]);
		this.biomeLists[1].addAll(RTGBiomeManager.overworldBiomes[1]);
		this.biomeLists[2].addAll(RTGBiomeManager.overworldBiomes[2]);
		this.biomeLists[3].addAll(RTGBiomeManager.overworldBiomes[3]);
	...
	public int[] getInts(int x, int z, int width, int length)
    {
        ...
        for (int i1 = 0; i1 < length; ++i1)
        {
            for (int j1 = 0; j1 < width; ++j1)
            {
                ...
                if (ConfigRTG.generateOnlyThisVanillaBiome.length() > 0)
                {
                	outputBiomeIDs[j1 + i1 * width] = RealisticBiomeVanillaBase.getRealisticVanillaBiomeFromVanillaVariableName(ConfigRTG.generateOnlyThisVanillaBiome).baseBiome.biomeID;
                	continue;
                }
                ...
                else if (currentBiomeID == BiomeGenBase.mushroomIsland.biomeID && ConfigRTG.generateVanillaMushroomIsland)
                {
                    outputBiomeIDs[j1 + i1 * width] = currentBiomeID;
                }
                else if (currentBiomeID == 1)
                {
                    if (l1 > 0)
                    {
                        if (this.nextInt(3) == 0 && ConfigRTG.generateVanillaMesaPlateau)
                        {
                            outputBiomeIDs[j1 + i1 * width] = BiomeGenBase.mesaPlateau.biomeID;
                        }
                        else if (ConfigRTG.generateVanillaMesaPlateau_F)
                        {
                ...
                {
                    if (l1 > 0 && ConfigRTG.generateVanillaJungle)
                    {
                ...
                {
                    if (l1 > 0 && ConfigRTG.generateVanillaMegaTaiga)
                    {
    ...
    {
        return (biomeId == BiomeGenBase.ocean.biomeID && ConfigRTG.generateVanillaOcean) || (biomeId == BiomeGenBase.deepOcean.biomeID && ConfigRTG.generateVanillaDeepOcean);
    }
}
