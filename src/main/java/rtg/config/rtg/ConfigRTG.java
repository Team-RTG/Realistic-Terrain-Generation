...
public class ConfigRTG extends ModConfig {
    ...
    public void setDefaults() {
        ...
        enableRTGBiomeDecorations = config.getBoolean(
                ...
                "If TRUE, uses the individual config settings in the config config files. If FALSE, disables all RTG decorations and uses vanilla decorations instead."
                        + Configuration.NEW_LINE
        );
        ...
        enableRTGBiomeSurfaces = config.getBoolean(
                ...
                "If TRUE, uses the individual config settings in the config config files. If FALSE, disables all RTG surfaces and uses vanilla surfaces instead."
                        + Configuration.NEW_LINE
        );
        ...
        patchBiomeId = config.getInt(
                ...
                -1,
                255,
                "If RTG tries to generate an unsupported config or a config that has an ID conflict, it will generate this config instead."
                        + Configuration.NEW_LINE +
                        "If set to -1, RTG will crash instead of generating the patch config. You might want to do this if you're making a mod pack"
                        + Configuration.NEW_LINE +
        ...
        useBOPLayouts = config.getBoolean(
                ...
                "Biomes",
                useBOPLayouts,
                "If set to false, this will use vanilla config layouts, even if BOP is installed. That means no BOP biomes!" +
                        Configuration.NEW_LINE +
        ...
        allowTreesToGenerateOnSand = config.getBoolean(
                ...
                "Set this to FALSE to prevent trees from generating on sand."
                        + Configuration.NEW_LINE +
                        "This setting only affects trees generated by RTG. Trees generated by a config's decorator"
                        + Configuration.NEW_LINE +
